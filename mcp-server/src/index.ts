...
if (!apiKey && !serviceJwt) throw new Error('Set MCP_API_KEY or MCP_SERVICE_JWT');
...
async function handlePost(request: Request, response: Response) {
  ...
  if (sessionId) {
    ...
    if (!session) return sendInvalidSession(response);
    return session.transport.handleRequest(request, response, request.body);
  }
  if (!isInitializeRequest(request.body)) return sendInvalidSession(response);
...
function buildServer() {
  ...
  server.registerResource(
    ...
    { title: 'Semaine courante', description: 'Menu et message de la semaine courante.', mimeType: 'application/json' },
    async (uri) => resource(uri.href, await backend('GET', '/mcp/week-data')),
  );
  server.registerResource(
    'week-by-start-date',
    new ResourceTemplate('miam-dv://weeks/{weekStart}', { list: undefined }),
    { title: 'Semaine par date', description: 'Menu et message d’un lundi YYYY-MM-DD.', mimeType: 'application/json' },
    async (uri, { weekStart }) => {
      const parsed = weekStartSchema.safeParse(weekStart);
      if (!parsed.success) throw new Error('weekStart doit être un lundi YYYY-MM-DD.');
...
async function backend(method: 'GET' | 'POST' | 'PUT' | 'DELETE', path: string, body?: unknown) {
  ...
  if (!response.ok) throw new Error(`Backend ${method} ${path} failed (${response.status}): ${typeof data === 'string' ? data : JSON.stringify(data)}`);
  return data;
}
function result(message: string, data: unknown) {
  return { content: [{ type: 'text' as const, text: `${message}\n\n${JSON.stringify(data, null, 2)}` }] };
}
function resource(uri: string, data: unknown) {
...
function sendInvalidSession(response: Response) {
  response.status(400).json({ jsonrpc: '2.0', error: { code: -32000, message: 'Bad Request: missing or invalid MCP session ID.' }, id: null });
}
async function guardMcpRequest(operation: () => Promise<unknown>, response: Response) {
...
function originGuard(request: Request, response: Response, next: NextFunction) {
  if (request.headers.origin && allowedOrigins.length && !allowedOrigins.includes(request.headers.origin)) return response.status(403).send('Origin not allowed');
  next();
}
...
function parseCsv(value?: string) { return value?.split(',').map((entry) => entry.trim()).filter(Boolean) ?? []; }
function parsePort(value: string) {
  const port = Number.parseInt(value, 10);
  if (!Number.isInteger(port) || port < 1 || port > 65535) throw new Error('MCP_HTTP_PORT must be a valid TCP port.');
  return port;
}
...
function errorMessage(error: unknown) { return error instanceof Error ? error.message : 'unreachable'; }
